...
"""
@File    :   dep_mul_bert_rule_rule_temp
@Time    :   2019-07-12
@Software:   PyCharm
@Author  :   Li Xiang
@Desc    :
...
app = DeployClient(module_name=__name__, port=7779, max_items=1000, expire_sec=600)

# 这里进行模型的实例化或一些模型必要的加载和初始化工作
nlu = BERTNLU('usr', model_file=os.path.join(root_dir, 'res/bert_multiwoz_usr.zip'))
policy = Rule()
nlg = TemplateNLG(is_user=False)
state_ini = copy.deepcopy(RuleDST().state)


@app.inference_buffer('multiwoz_bert_rule_rule_temp')
